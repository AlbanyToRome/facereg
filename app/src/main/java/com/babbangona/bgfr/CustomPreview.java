...
class CustomPreview extends SurfaceView implements Callback {
     ...
     */
    CustomPreview(Context paramContext, CustomProcess paramProcessImageAndDrawResults)
    {
        super(paramContext);
    ...
    public void surfaceChanged(SurfaceHolder paramSurfaceHolder, int paramInt1, int paramInt2, int paramInt3)
    {
       ...
       /* //TODO: TEST THIS ALTERNATIVE AS LARGEST SIZE
        width                                               = (int)(0.975*supportedSizes.get(supportedSizes.size()-1).width);
        height                                              = (int)(0.975*supportedSizes.get(supportedSizes.size()-1).height);*/
    ...
    public void switchFlash(){
        ...
        Log.i(TAG, "switchFlash: FLASH_MODE  == "+ P.getFlashMode());
        if (P.getFlashMode() == null)
            return;
        if (P.getFlashMode().contains(Camera.Parameters.FLASH_MODE_OFF))
        {
            Log.i(TAG, "switchFlash: FLASH_MODE  == "+ P.getFlashMode());
            ...
            this.mCamera.setPreviewCallback(new PreviewCallback()
            {
                public void onPreviewFrame(byte[] paramAnonymousArrayOfByte, Camera paramAnonymousCamera)
                {
                    if (mDraw != null)
                    {
                        ...
                        System.arraycopy(paramAnonymousArrayOfByte, 0, mDraw.mYUVData, 0, paramAnonymousArrayOfByte.length);
                        mDraw.invalidate();
                    }
                }
            });
